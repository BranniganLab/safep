...
def do_estimation(u_nk: pd.DataFrame, method: str="both"):
    ...
    if method in ["both", "BAR"]:
        bar_per_window, bar_cumulative = do_bar(u_nk)
    ...
    if method in ["both", "EXP"]:
        exp_per_window, exp_cumulative = do_exp(u_nk)
...
def do_exp(u_nk: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Do exponential averaging and repackage the results into a more convenient dataframe.

    Arguments: u_nk in the alchemlyb format
    Returns: cumulative estimates (including errors), per window estimates (including errors)
    """
    ...
    return exp_per_window, exp_cumulative


def do_bar(u_nk: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Do bar estimation and repackage the results into a more convenient dataframe.
    ...
    return bar_per_window, bar_cumulative
