...
def parse_Colvars_log(filename):
    ...
    for line in lines:
        ...
        match = re.match(
            r'^colvars: The final output state file will be "(.+).colvars.state".$',
            line,
        )
        if match:
        ...
        match = re.match(
            r"^colvars:\s+Restraint (\S+), stage (\S+) : lambda = (\S+), k = (\S+)$",
            line,
        )
        if match:
        ...
        if match:
            ...
            if TI_traj[name]["L"][-1] != L:
                print(
                    f"Error: mismatched lambda value in log: expected lambda = {L} and read:\n{line}"
                )
                break
        ...
        match = re.match(
            r'^colvars: Synchronizing \(emptying the buffer of\) trajectory file "(.+)"\.$',
            line,
        )
        if match:
...
def initialize_global_conf(lines):
    ...
    for line_number, line in enumerate(lines):
        match = re.match(
            r"^colvars: Initializing the collective variables module, version (.*).$",
            line,
        )
        if match:
