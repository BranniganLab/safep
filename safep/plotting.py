...
def convergence_plot(
    ...
    theax.plot(
        0, final_mean, linewidth=1, color=fwd_legend, label="Forward Time Sampling"
    )
    theax.plot(
        0,
        final_mean,
...
def plot_general(
    ...
    if fig is None and axes is None:
        fig, axes = plt.subplots(
            3, 2, sharex="col", sharey="row", gridspec_kw={"width_ratios": [2, 1]}
        )
    ...
    cumul_ax.set(
        ylabel=r"Cumulative $\mathrm{\Delta} G_{\lambda}$" + "\n(kcal/mol)",
    ...
    if errorbars:
        each_ax.errorbar(
            per_window.index,
            per_window.BAR.df * RT,
            yerr=per_window.BAR.ddf,
            marker=None,
            linewidth=1,
            alpha=0.5,
            color=color,
    ...
    each_ax.set(
        ylabel=r"$\mathrm{\Delta} G_\lambda$"
        + "\n"
        + r"$\left(\mathrm{kcal}/\mathrm{mol}\right)$",
        ylim=per_window_ylim,
    )

    # Hysteresis Plots
    hyst_ax, pdf_ax = plot_hysteresis(
        (hyst_ax, pdf_ax), per_window, hysttype, color, fontsize, pdf_type, xlim
    )
...
def add_hyst_textbox(diff, pdf_x, pdf_y, pdf_ax, xoffset=0.15, yoffset=0.95, fs=14):
    ...
    textstr = (
        rf"$\rm mode=${np.round(mode,2)}"
        + "\n"
        + rf"$\mu$={np.round(mean,2)}"
        + "\n"
...
def plot_hysteresis(
    ...
    assert hysttype in [
        "classic",
        "lines",
    ], f"ERROR: I don't know how to plot hysttype={hysttype}"
    if hysttype == "classic":
        hyst_ax.vlines(
            per_window.index,
            np.zeros(len(per_window)),
            diff,
            label="fwd - bwd",
            linewidth=2,
            color=color,
        ...
        hyst_ax.plot(
            per_window.index, diff, label="fwd - bwd", linewidth=1, color=color
        )
...
def plot_general_legacy(
    ...
    """
    fig, axes = plt.subplots(
        4, 2, sharex="col", sharey="row", gridspec_kw={"width_ratios": [2, 1]}
    )
    (
        (cumulative_ax, del1),
        (each_ax, del2),
        (ddG_ax, del3),
        (hysteresis_ax, pdf_ax),
    ) = axes
    ...
    each_ax.errorbar(
        perWindow.index,
        perWindow.BAR.df * RT,
        yerr=perWindow.BAR.ddf,
        marker=None,
        linewidth=1,
    )
    each_ax.plot(
        perWindow.index, perWindow.EXP.dG_f * RT, marker=None, linewidth=1, alpha=0.5
    )
    each_ax.errorbar(
        perWindow.index, -perWindow.EXP.dG_b * RT, marker=None, linewidth=1, alpha=0.5
    )
    each_ax.set(
    ...
    hysteresis_ax.vlines(
        perWindow.index, np.zeros(len(perWindow)), diff, label="fwd - bwd", linewidth=2
    )
