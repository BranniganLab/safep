...
def getDiscrepancies(dG_f, dG_b):
    '''
    Get the forward-backward discrepancies from the exponential estimations
    '''
    return dG_f + np.array(dG_b)

def getCDF(data):
    '''
    Estimate the CDF of the data using a finite sum
    Arguments: a list of data
    Returns: estimated cdfX and cdfY
    '''
    cdfX = data.copy
    cdfX.sort()
    lenX = len(cdfX)
    cdfY = np.arange(lenX)/lenX
    
    return cdfX, cdfY
    

#Calculate the PDF of the discrepancies
def getHist(data, binNum=20):
    '''
    Estimate the PDF of the discrepancy data by a histogram
    Arguments: a list of data, binNum (number of bins for histogramming)
    Returns: X and Y for plotting a histogram of the data
    '''
    histY, histX = np.histogram(data, bins=binNum, density=True)
    histX = (histX[1:]+histX[:-1])/2
    #pdfYexpected = norm.pdf(pdfX, fitted[0], fitted[1])
           
    return histX, histY

def getKDE(dG_f, dG_b, _):
    '''
    Estimate the PDF of the discrepancy data by a gaussian kernel density estimator
    Arguments: forward and backward exponential estimator data (and a placeholder dummy variable)
    Returns: diff, Y, kdeX, kdeY
    '''


def fitGaussian(X, Y=None, fitting='LS'):
    '''
    Fit a normal distribution to the data
    Arguments: X, Y, fitting method
    ...
    if DiscrepancyFitting == 'LS':
        try:
            ...
            print("Must include Y data for least squares fitting")
            raise
...
def genGaussian(X, fitted, dx=0.01):
    '''
    Generate a PDF gaussian curve given fitted parameters
    Arguments: DiscrepancyFitting ('LS' least squares of 'ML' maximum likelihood), dx (sample width for fitted gaussian)
    Returns: X and Y data for plotting the idealized curve
    '''
    pdfXnorm  = np.arange(np.min(X), np.max(X), dx)
    pdfYnorm = norm.pdf(pdfXnorm, fitted[0], fitted[1])

    return pdfXnorm, pdfYnorm


def getGaussian(DiscrepancyFitting='LS', dx=0.01):
    '''
    Arguments: fitted parameters, (fitted) pdfXnorm, pdfYnorm, pdfYexpected (for each experimental X) 
    '''
    fitted = fitGaussian(X, Y, DiscrepancyFitting)
    pdfXnorm, pdfYnorm = genGaussian(X, fitted, dx)
    
    return fitted, pdfXnorm, pdfYnorm
